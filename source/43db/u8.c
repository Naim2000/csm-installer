...
#define ERROR_MSG(str, ...) fprintf(stderr, "%s:%i: " str "\n", __FILE__, __LINE__, ##__VA_ARGS__)
...
bool u8ContextInit(void *buf, U8Context *ctx)
{
    ...
    if (u8_header.magic != U8_MAGIC || u8_header.root_node_offset <= (u32)sizeof(U8Header) || u8_header.node_info_block_size <= (u32)sizeof(U8Node) || \
        u8_header.data_offset != __builtin_align_up(u8_header.root_node_offset + u8_header.node_info_block_size, U8_FILE_ALIGNMENT))
    {
    ...
    str_table = (char*)calloc(sizeof(char), str_table_size);
    if (!str_table)
    {
